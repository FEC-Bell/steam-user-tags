...
const seedValues = () => {
  ...
  for (let gameID = 1; gameID <= 100; gameID += 1) {
    ...
    const gameTagCount = rnd(40) + 10;

    for (let tags = 0; tags < gameTagCount; tags += 1) {
      // pick a random tag index
      const tagIndex = rnd(tagList.length - 1);
      ...
      if (!gameTags.includes(tagIndex)) {
        ...
        // count calc is random, but also weighted by tag popularity ( tagList.length-tagIndex )
        // count / 4 for smaller, more plausible numbers; decimals are implicitly converted to ints
